...
impl Ed25519 {
    ...
    /// group the base point generates. RFC 8032 §8.4 warns that a signature
    /// under such a key verifies under every other key of the same tiny order,
    /// so it proves nothing about who holds the private key. The cofactor of
    /// edwards25519 is 8, so three doublings settle it.
...
impl Ed448 {
    ...
    /// the Ed25519 check: RFC 8032 §8.4 warns that a signature under a
    /// small-order key verifies under every other key of the same order. The
    /// cofactor of edwards448 is 4, so two doublings settle it.
...
mod tests {
    ...
    fn a_small_order_public_key_is_malformed() {
        ...
        // clear. It has order 1, so RFC 8032 §8.4's warning applies and the
        // key is refused before the signature equation is reached.
        let mut key = vec![0u8; 32];
        key[0] = 1;
        let signature = hex_bytes(TEST_1.signature);
        let challenge = hex_bytes(TEST_1.challenge);
    ...
    fn a_small_order_ed448_public_key_is_malformed() {
        ...
        // 57th. Order 1, so RFC 8032 §8.4's warning applies here too.
        let mut key = vec![0u8; 57];
        key[0] = 1;
        let signature = hex_bytes(ED448_BLANK.signature);
        let challenge = hex_bytes(ED448_BLANK.challenge);
