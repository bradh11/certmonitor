...
- `revocation`: a CRL is cached only once its signature has verified (`remember_crl`); `fetch_crl` no longer caches, so a corrupted or forged CRL is fetched again next time instead of being served from the cache until its `nextUpdate`.
