...
- Opt-in `revocation` validator: checks the certificate's OCSP responder and CRL distribution points. CRL answers are verified by OpenSSL through one extra handshake with the CRL loaded; OCSP answers are parsed by the in-house DER parser and reported with `signature_verified: false` until in-tree signature verification lands, so an OCSP-only `good` is a warning unless `accept_unverified=True`. Fetches go through the monitor's timeout and proxy and are cached until `nextUpdate` (#90).
