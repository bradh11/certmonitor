...
!!! tip "Count connections as well as hosts"
    Protocol detection, collection, verified trust, and an enabled PQ probe each make their own connection, once per observation. A STARTTLS service adds the failed handshake attempts and a discovery exchange the first time it is met; pass `starttls` for endpoints you already know to skip both. Account for those when choosing a rate limit, especially against a small appliance or a shared service.
