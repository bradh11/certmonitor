...
- 🚫 **Revocation:** the opt-in `revocation` validator asks the certificate's OCSP responder and CRL distribution points and verifies both answers, both with in-house RSA and ECDSA signature checks against an issuer that provably signed the certificate. See [Revocation](validators/revocation.md).
