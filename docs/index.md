...
- 📮 **STARTTLS too:** `starttls="smtp"` (or `imap`, `pop3`, `ftp`, `postgres`, `ldap`) runs the service's preamble first, so mail, directory, and database certificates get the same checks. See [STARTTLS Services](usage/starttls.md).
