...
!!! note "How verification stays dependency-free"
    The Python standard library has no primitive for checking an RSA, ECDSA, or EdDSA signature, so CertMonitor's Rust extension carries its own: big-integer arithmetic, RSASSA-PKCS1-v1_5, RSASSA-PSS (MGF1 with SHA-1, SHA-256, SHA-384, or SHA-512), ECDSA over P-256, P-384, and P-521, and Ed25519 and Ed448. It is verification only, tested against RFC 6979 and RFC 8032 vectors, Wycheproof's ECDSA, RSA, RSASSA-PSS, and EdDSA suites, and real `openssl` output. What remains unsupported is anything else, for example post-quantum signatures, SHAKE-based PSS, or other curves; those are reported with `signature_verified: false` and the algorithm named in `verification_error`. An RSA signer whose key is encoded with `id-RSASSA-PSS` rather than `rsaEncryption` ([RFC 4055 section 1.2](https://www.rfc-editor.org/rfc/rfc4055.html#section-1.2)) is accepted, and the restrictions such a key carries are enforced: it verifies only RSASSA-PSS signatures whose hash and mask generation function match the key's and whose salt is at least as long ([section 3.3](https://www.rfc-editor.org/rfc/rfc4055.html#section-3.3)); a PKCS#1 v1.5 signature under such a key violates that same section and is a hard failure that `accept_unverified` does not rescue.
...
- **OCSP** is one small HTTP `POST` to the responder. The request is built from the certificate's serial and its issuer's name and key, so the issuer certificate is needed; it comes from the served chain, or from the certificate's `caIssuers` pointer when the server sends only the leaf. That pointer may serve a single certificate or a certs-only PKCS#7 bundle (RFC 5280 section 4.2.2.1), in DER or PEM; every certificate in it is tried as the issuer. Verifying the response takes a few milliseconds of arithmetic.
...
- **Both are cached** process-wide until their `nextUpdate` and never a second longer: the cache reuses signed evidence only while the signer said it was valid. A fleet scan therefore fetches each CA's CRL and each certificate's OCSP answer once per validity window. Set `methods=["crl"]` when many hosts share a CA and you want one download for all of them. An OCSP response without `nextUpdate` is accepted for `max_age_hours` (24 by default) after its `thisUpdate` and never cached past that; responses older than ten days are refused outright. A CRL without `nextUpdate` is refused once its `thisUpdate` is older than ten days, like an OCSP response. Answers whose signature failed verification are never cached, and a CRL is cached only once its signature has verified, so a corrupted copy is fetched again next time instead of being pinned until its `nextUpdate`. What the OCSP cache holds is the responder's evidence, not a verdict about one certificate: each time an answer is reused it is held to the same freshness rule as a new response, so nothing is served once its `thisUpdate` is ten days old or once the delegated responder certificate that signed it has expired, and it is capped by how well the certificate being checked is bound to its issuer, so a cache hit never inherits an earlier check's `signature_verified`.
