...
- 🚫 **Revocation:** the opt-in `revocation` validator asks the certificate's OCSP responder and CRL distribution points and verifies both answers, both with in-house RSA (PKCS#1 v1.5 and PSS), ECDSA (P-256, P-384, P-521), and EdDSA (Ed25519, Ed448) signature checks against an issuer that provably signed the certificate. See [Revocation](https://certmonitor.readthedocs.io/en/latest/validators/revocation/).
