...
def test_fetch_raw_cert_includes_public_keys(cert_monitor):
    """Test that _fetch_raw_cert includes public key data in cert_data."""
    mock_der = b"mock der data"
    mock_pem = "-----BEGIN CERTIFICATE-----\nmock pem data\n-----END CERTIFICATE-----\n"
    ...
    # Verify the result includes all public key data
    assert isinstance(result, dict)
    assert result["public_key_info"] == mock_public_key_info
    assert result["public_key_der"] == mock_public_key_der
    assert result["public_key_pem"] == mock_public_key_pem

    # Verify instance attributes are set
    assert cert_monitor.public_key_der == mock_public_key_der
    assert cert_monitor.public_key_pem == mock_public_key_pem
    assert cert_monitor.public_key_info == mock_public_key_info


def test_fetch_raw_cert_public_key_parsing_error(cert_monitor):
    """Test that _fetch_raw_cert handles public key parsing errors gracefully."""
    mock_der = b"mock der data"
    mock_pem = "-----BEGIN CERTIFICATE-----\nmock pem data\n-----END CERTIFICATE-----\n"
    mock_cert_info = {"subject": {"commonName": "example.com"}}
    ...
    # Mock the certinfo functions to raise an exception
    with patch("certmonitor.core.certinfo") as mock_certinfo:
        mock_certinfo.parse_public_key_info.side_effect = Exception("Parsing failed")
    ...
    assert (
        "Failed to parse public key info: Parsing failed"
        in result["public_key_info"]["error"]
    )
    assert result["public_key_der"] is None
    assert result["public_key_pem"] is None

    # Verify instance attributes are set to None
    assert cert_monitor.public_key_der is None
    assert cert_monitor.public_key_pem is None


def test_fetch_raw_cert_no_der_data(cert_monitor):
    """Test that _fetch_raw_cert handles missing DER data gracefully."""
    mock_pem = "-----BEGIN CERTIFICATE-----\nmock pem data\n-----END CERTIFICATE-----\n"
    mock_cert_info = {"subject": {"commonName": "example.com"}}

    # Mock the handler's fetch_raw_cert to return data without DER
    cert_monitor.handler.fetch_raw_cert.return_value = {
        "cert_info": mock_cert_info,
        "der": None,  # No DER data
        "pem": mock_pem,
    }
    ...
    # Verify the result includes error information for missing DER
    assert isinstance(result, dict)
    assert "public_key_info" in result
    assert "error" in result["public_key_info"]
...
def test_get_public_key_der_triggers_fetch_when_none(cert_monitor):
    """Test that get_public_key_der triggers certificate fetch when public_key_der is None."""
    mock_der = b"mock der data"
    mock_public_key_der = b"mock public key der"
    cert_monitor.public_key_der = None  # Initially None

    # Mock _fetch_raw_cert to return cert data and set public_key_der
    def mock_fetch_raw_cert():
        cert_monitor.public_key_der = mock_public_key_der
        return {"der": mock_der, "public_key_der": mock_public_key_der}
...
def test_get_public_key_pem_triggers_fetch_when_none(cert_monitor):
    """Test that get_public_key_pem triggers certificate fetch when public_key_pem is None."""
    mock_pem = "-----BEGIN CERTIFICATE-----\nmock pem data\n-----END CERTIFICATE-----\n"
    mock_public_key_pem = (
        "-----BEGIN PUBLIC KEY-----\nmock key\n-----END PUBLIC KEY-----"
    )
    cert_monitor.public_key_pem = None  # Initially None

    # Mock _fetch_raw_cert to return cert data and set public_key_pem
    def mock_fetch_raw_cert():
        cert_monitor.public_key_pem = mock_public_key_pem
        return {"pem": mock_pem, "public_key_pem": mock_public_key_pem}
...
def test_ensure_connection_returns_none_on_success(cert_monitor):
    """Test that _ensure_connection returns None when connection is successful."""
    cert_monitor.connected = False

    with patch.object(cert_monitor, "connect", return_value=None):
        result = cert_monitor._ensure_connection()
        assert result is None


def test_ensure_connection_returns_error_on_failure(cert_monitor):
...
def test_ensure_connection_reconnects_on_lost_connection(cert_monitor):
...
def test_connect_protocol_detection_error():
...
def test_connect_ssl_handler_creation():
...
def test_connect_ssh_handler_creation():
...
def test_connect_handler_connection_error():
...
def test_detect_protocol_ssl_handshake():
...
def test_detect_protocol_ssl_alternative_bytes():
...
def test_detect_protocol_unknown_data():
...
def test_detect_protocol_no_data_defaults_ssl():
...
def test_detect_protocol_connection_error():
...
def test_is_ip_address_invalid():
    ...
    # Test invalid IP address - this should return False and cover the except block on line 134
    assert not monitor._is_ip_address("not.an.ip.address")
    assert not monitor._is_ip_address("999.999.999.999")
    assert not monitor._is_ip_address("invalid_host")

    # Test valid IP addresses
    assert monitor._is_ip_address("192.168.1.1")
    assert monitor._is_ip_address("2001:db8::1")


def test_is_ip_address_exception_coverage():
    """Test _is_ip_address with values that raise ValueError to specifically cover line 134."""
    monitor = CertMonitor("example.com")

    # These should trigger the ValueError exception on line 134
    test_cases = [
    ...
    for test_case in test_cases:
        result = monitor._is_ip_address(test_case)
        assert result is False  # All should return False when ValueError is caught
...
def test_is_ip_address_with_invalid_input():
    """Test _is_ip_address method with invalid input to cover line 134."""
    monitor = CertMonitor("example.com")

    # Test with invalid IP address formats
    assert not monitor._is_ip_address("not.an.ip.address")
    assert not monitor._is_ip_address("invalid")
    assert not monitor._is_ip_address("256.256.256.256")  # Invalid IP
    assert not monitor._is_ip_address("")  # Empty string
